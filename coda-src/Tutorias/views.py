...
def generar_pdf(request):
    ...
    # Agregar nombre del tutor
    tutor_name = f'Nombre Tutor: {tutor_loggeado.nombre_completo}'
    tutor_name_paragraph = Paragraph(tutor_name, header_style)
    elements.append(tutor_name_paragraph)
    ...
    for tutoria in tutorias_tutor:
        data.append([
            tutoria.alumno.nombre_completo,
...
def generar_archivo_txt(request,pk):
    ...
    for tutoria in tutorias:
        contenido += f"Alumno: {tutoria.alumno.nombre_completo}\n"
        contenido += f"Tutor: {tutoria.tutor.nombre_completo}\n"
...
class ReporteTutoriasBrindadasView(CodaViewMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        plantilla = get_object_or_404(Documento, nombre=plantilla_nombre, tipo='reporte')

        from Usuarios.services.plantillas_documentos import validar_archivo
        try:
            validar_archivo(plantilla.archivo_fuente, 'reporte')
        except forms.ValidationError as error:
            formulario = self.get_form()
            formulario.is_valid()
            formulario.add_error('plantilla', error)
            self.object = None
...
class ReporteTutoriasBrindadasMasivoView(CodaViewMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            form.add_error('plantilla', error)
            return self.form_invalid(form)
...
class VerTutoresListView(CodaViewMixin, ListView):
    ...
    def get_queryset(self):
        tutores = tutores_administrables(self.request.user)
        licenciatura = self.request.GET.get('licenciatura', '')
        if licenciatura:
            tutores = tutores.filter(coordinacion=licenciatura)
        return tutores

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['licenciaturas'] = [
            (codigo, nombre) for codigo, nombre in Tutor._meta.get_field('coordinacion').choices if codigo
        ]
        context['licenciatura_seleccionada'] = self.request.GET.get('licenciatura', '')
        return context
